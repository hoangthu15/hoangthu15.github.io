...
return $id;
  ...
  function get_thumb_freefire($id) {
    ...
    public function check_user_nhan_qua() {
        $user = $this->user();
        $result = mysqli_query($this->connect_db(), "SELECT * FROM `register_gift` WHERE `username`='".$user['username']."'");
        $row = mysqli_num_rows($result);
...
function phantrang($url, $start, $total, $kmess) {
    $out[] = '<div class="row"><div class="col-md-12"><center>';
    $neighbors = 2;
